...
public class CheckboxListTag extends ItemTag {
	...
	public int doStartTag() throws JspException {
		...
		if (list.startsWith("#{") && list.endsWith("}") && list.length() > 2) {
			list = list.substring(0, list.length() - 1); //NOPMD
			list = list.substring(2); //NOPMD
			...
			if (items != null) {
				for (int i = 0; i < items.length; i++) {
					...
					if (id != null) {
						buffer.append(" id=\"").append(id).append(item[0]).append("\"");
					}
					if (item[0] != null) {
					...
					buffer.append(">");
					if (id != null) {
						buffer.append("<label for=\"").append(id).append(item[0]).append("\">").append(item[1]).append(
								"</label>");
					} else {
			...
			if (obj instanceof Map) {
				...
				while (iterator.hasNext()) {
					...
					buffer.append(commonHTML);
					if (id != null) {
						buffer.append(" id=\"").append(id).append(keyText).append("\"");
					}
					if (keyText != null) {
					...
					if (id != null) {
						buffer.append("><label for=\"").append(id).append(keyText).append("\">").append(valueText)
								.append("</label>");
					} else {
