...
@Repository(value = "sysCommonDao")
public class SysCommonDaoHibernate extends EntityDaoHibernate {
	...
	public Page findBySql(final Class classType,String sql,Integer pageNo,Integer pageSize,final Object values[]){
		...
		StringBuffer sqlbuf = new StringBuffer("select count(*) from (").append(fnSql).append(") A");
		...
		return new Page(startIndex, ((BigInteger)totalCount.get(0)).longValue(), pageSize.intValue(), list);
	}
...
//		    modifyHql = " select count(1) from (" + removeSelect(fnHql) + ") tt";
		}
