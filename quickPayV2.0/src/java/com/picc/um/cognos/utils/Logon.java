...
public class Logon{
	...
	public static String COGNOSDISPATCHER;
	public static String THIRDPJNAMESPACE = "dbAuth";
	...
	public String logon(CRNConnect connection) {
		...
		if (namespaceInfo == null) {
			logger.error("Unable to connect to Cognos 10, Connect Failed.");
			
			return "Unable to connect to Cognos 10";
		}
	...
	public  boolean loggedIn(CRNConnect connection) {
	...
	public String logonInfo(CRNConnect connection) {
		...
		if (connection != null) {
			Account myAccount = getLoggeronAccount(connection);
			if (myAccount == null) {
	...
	public static boolean doTestForAnonymous(CRNConnect connection) {
		...
		try {
			...
			logger.info("Anonymouse access is denied");
			////System.out.println("");
	...
	public static Account getLoggeronAccount(CRNConnect connection) {
		...
		if (connection.getCMService() == null) {
			logger.error("Invalid parameter passed to function logon.");
			return myAccount;
		}
		...
		try {
			...
			logger.error("Caught RemoteException:\n" + remoteEx.getMessage());
		}
