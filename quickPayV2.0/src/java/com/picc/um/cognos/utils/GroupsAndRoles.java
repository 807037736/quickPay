...
public class GroupsAndRoles {
	...
	private static final Logger logger = LoggerFactory.getLogger(GroupsAndRoles.class);
	 ...
	 * @author shenyichan
	 */
	public List<Account> getAvailableUsers(CRNConnect connection,
			String pathOfRole) {
		try {
			List<Account> bcList = new ArrayList<Account>();
			PropEnum[] props = { PropEnum.defaultName, PropEnum.searchPath,
					PropEnum.members };

			Role role = (Role) csHandler.queryObjectInCS(connection,
					pathOfRole, props)[0];
			if (role.getMembers().getValue() != null) {
				BaseClass obj = null;
				int l = role.getMembers().getValue().length;
				for (int i = 0; i < l; i++) {
					obj = role.getMembers().getValue()[i];
					if (obj instanceof Account) {
						bcList.add((Account) obj);
					}
				}
				return bcList;
			}
	...
	public BaseClass[] getAvailableRoles2(CRNConnect connection,
			String namespace) {
		PropEnum props[] = new PropEnum[] { PropEnum.searchPath,
				PropEnum.defaultName };
		BaseClass[] availableRoles;
	 ...
	 * @param role
	 */
	public void updateCognosRole(CRNConnect connection, String pathOfRole,
			UmTRole role) {
		PropEnum[] props = { PropEnum.defaultName, PropEnum.searchPath,
				PropEnum.members };

		BaseClass[] bc = getAvailableRoles2(connection, pathOfRole);

		if (bc != null && bc.length > 0) {
			try {
				for (BaseClass r : bc) {
					TokenProp tp = r.getDefaultName();
					tp.setValue(role.getRoleCName());
					r.setDefaultName(tp);
					// MultilingualToken[] mt = r.getName().getValue();
					// mt[0].setValue(role.getRoleCName());
					// r.getName().setValue(mt);
				}
				csHandler.updateObjectInCS(connection, bc);
