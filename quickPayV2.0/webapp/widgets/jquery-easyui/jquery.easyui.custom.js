...
*选择一行记录。
*/
function getSingleSelectRow(dataTableId,idField, errorMessage) {
    var rows = $('#' + dataTableId).datagrid('getSelections');
    var num = rows.length;
    var id = null;
    if (num == 1) {
    	id = eval("rows[0]."+idField);
        return id;
    } else {
...
* 注：该方法使用的前提是：DataGrid的idField属性对应到列表Json数据中的字段名用idField传入
* @paramdataTableId目标记录所在的DataGrid列表table的id
*
* @return 所选记录的id字符串(多个id用逗号隔开)
*/
function getSelectIds(dataTableId,idField, noOneSelectMessage) {
    var rows = $('#' + dataTableId).datagrid('getSelections');
    var num = rows.length;
    var ids = null;
    if (num < 1) {
...
/**
 * 对Ajax的数据请求请行权限校验,如果越权操作则进行错误提示
 */
$(document).ajaxComplete(function(event, resp, settings){
	if(resp&&resp.getResponseHeader){
		if(resp.getResponseHeader("_forbidden")){
			var url;
			if(settings.url.indexOf("?")!=-1){
				url = settings.url.substring(0,settings.url.indexOf("?"));
			}else {
				url = settings.url;
			}
			$.messager.alert("错误提示","您没有权限访问请求<br />"+url);
		}
	}
});

/**
 * 根据后台返回的结果result，判断result中是否包含errorTitle和errorMsg，如果是则根据errorTitle和errorMsg提示，如果不是则按默认提示信息提示。
 */
function alertErrorMsgForEasyUi(result){
	if(result.responseText!=null){
		var jsonObject;
		try{
			jsonObject = eval('('+result.responseText+')');
			if(jsonObject!=null&&jsonObject.errorTitle!=null&&jsonObject.errorMsg!=null){
				$.messager.alert(jsonObject.errorTitle,jsonObject.errorMsg);
			}else{
				$.messager.alert(datagridErrorTitle,datagridErrorMes);
			}
		}catch(e){}
	}else{
		$.messager.alert(datagridErrorTitle,datagridErrorMes);
