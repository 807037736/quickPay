	...
	setBirthSex:function(idcard){//从身份证设置生日及性别
		...
		switch(idcard.length){
			case 18:
            	...
            	$("input[name='app.birthday']").val(year+"-"+month+"-"+day);
            	if(sex%2==0){
	                $("input[name='app.gender']").eq(1).prop("checked",true);
	            }else{
	                $("input[name='app.gender']").eq(0).prop('checked',true);
	            }
	            $("input[name='app.birthday']").prop('disabled',true);
	            $("input:radio[name='app.gender']").prop("disabled",true);
				break;
			case 15:
	            ...
	            $("input[name='app.birthday']").val(year+"-"+month+"-"+day);            
	            if(sex%2==0){
	                $("input:radio[name='app.gender']").eq(1).prop("checked",true);
	            }else{
	                $("input:radio[name='app.gender']").eq(0).prop('checked',true);
	            }
	            $("input[name='app.birthday']").prop('disabled',true);
	            $("input:radio[name='app.gender']").prop("disabled",true);
				break;
	...
	idNoInput:function(idTypeSleId,idNoId,bridayId,sexName){
		$("#"+idTypeSleId).bind("change",function(){
			...
			if(idType=='01'){			
				...
				$("input[name='app.id_card_no']").val('');
				$('#appIdnoMsg').attr('class','sfValidatebox-hid');
				$('#appBirthdayMsg').attr('class','sfValidatebox-hid');				
				$("input[name='app.birthday']").val('');
				$("input[name='app.birthday']").removeProp('disabled',true);
	            $("input:radio[name='app.gender']").removeProp("disabled",true);
