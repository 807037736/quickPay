var query_action = contextRootPath + "/um/umtuserrole/query.do";
var queryResultTable = "UmTUserRoleTable";
var page_toolBar = [ {
	text : '修改',
	align : 'right',
	iconCls : 'icon-edit',
		...
		},
		{
			...
			formatter : function(value, row, index) {
				return '<a style="cursor:pointer;text-decoration: underline;" onclick="view(\''
						+ getUrlByJson(row) + '\')">' + row.id.userRoleId + '</a>';
			}
		},
		{
			field : 'roleId',
...
function prepareUpdate() {
	...
	var url = contextRootPath+"/um/umtuserrole/prepareUpdate.do?" + getUrlByJson(rows[0]);
	editRecord(url);
}
/* 新增 */
function prepareAdd() {
	editRecord(contextRootPath+'/um/umtuserrole/prepareAdd.do');
}
/* 查看 */
function view(params) {
	var url = contextRootPath+"/um/umtuserrole/view.do?" + params;
	editRecord(url);
}
...
function executeDelete() {
	...
	$.messager.confirm('提示', '是否确认删除?', function(r){
		if (r){
			var url = contextRootPath+"/um/umtuserrole/delete.do?" + getUrlByJson(rows[0]);
			 $.ajax({
					   type: "POST",
					   url: url,
	    	...
	    	$.messager.confirm('提示', '是否确认保存?', function(r){
				if (r){
				 	if($('#operateType').val()=='update'){
				 		$("#fm").attr("action",contextRootPath+"/um/umtuserrole/update.do");
				 		$("#fm").submit();
				 	}else if($('#operateType').val()=='add'){
				 		$("#fm").attr("action",contextRootPath+"/um/umtuserrole/add.do");
				 		$("#fm").submit();
				 	}else{
				 		return false;
				 	}				 	
