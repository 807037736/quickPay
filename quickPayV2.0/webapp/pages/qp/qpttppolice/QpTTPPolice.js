var query_action = contextRootPath + "/qp/qpttppolice/query.do";
var queryResultTable = "QpTTPPoliceTable";
var page_toolBar = [ {
	text : '修改',
	align : 'right',
	iconCls : 'icon-edit',
		...
		},
		{
			...
			formatter : function(value, row, index) {
				return '<a style="cursor:pointer;text-decoration: underline;" onclick="view(\''
						+ getUrlByJson(row) + '\')">' + row.id.policeId + '</a>';
			}
		},
		{
			field : 'policeName',
...
function prepareUpdate() {
	...
	var url = contextRootPath+"/qp/qpttppolice/prepareUpdate.do?" + getUrlByJson(rows[0]);
	editRecord(url);
}
/* 新增 */
function prepareAdd() {
	editRecord(contextRootPath+'/qp/qpttppolice/prepareAdd.do');
}
/* 查看 */
function view(params) {
	var url = contextRootPath+"/qp/qpttppolice/view.do?" + params;
	editRecord(url);
}
...
function executeDelete() {
	...
	$.messager.confirm('提示', '是否确认删除?', function(r){
		if (r){
			var url = contextRootPath+"/qp/qpttppolice/delete.do?" + getUrlByJson(rows[0]);
			 $.ajax({
					   type: "POST",
					   url: url,
	    	...
	    	$.messager.confirm('提示', '是否确认保存?', function(r){
				if (r){
				 	if($('#operateType').val()=='update'){
				 		$("#fm").attr("action",contextRootPath+"/qp/qpttppolice/update.do");
				 		$("#fm").submit();
				 	}else if($('#operateType').val()=='add'){
				 		$("#fm").attr("action",contextRootPath+"/qp/qpttppolice/add.do");
				 		$("#fm").submit();
				 	}else{
				 		return false;
				 	}				 	
